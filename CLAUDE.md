...
各セクションが空なら「該当なし」の 1 行を書く（見出しごと省略しない）。
...
`_config.yml` に `future: true` を設定済み。**削除・変更しないこと。**

記事の日付は `08:00:00 +0900`（JST）だが、UTC に換算すると前日の `23:00:00` になる。
GitHub Pages のビルドはコミット直後（UTC 深夜）に走るため、`future: false`（デフォルト）のままだと
「ビルド時刻より未来の投稿」と判定されサイトに表示されない。
